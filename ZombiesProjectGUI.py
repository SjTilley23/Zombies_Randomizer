...
import PygameTWF
import PygameCTBF
from VariousLists import (Challenge, bDescriptions, redGreen, Map, rangesList, extraInfoDesc,
...
while gameGoBRRR:
    ...
    if (pygame.mouse.get_pos()[0] >= 200 and pygame.mouse.get_pos()[0] <= 600 and pygame.mouse.get_pos()[1] >= 200 and
            pygame.mouse.get_pos()[1] <= 600 and secondScreen == False and previousClick == False and Click[0]):
    ...
    if secondScreen:
        ...
        # Redo Button
        if (pygame.mouse.get_pos()[0] >= 690 and pygame.mouse.get_pos()[0] <= 790 and pygame.mouse.get_pos()[1] >= 10 and
                pygame.mouse.get_pos()[1] <= 60 and previousClick == False and Click[0]):
            mapNumber = random.randint(0, 8)
            challengeNumber = random.randint(0, 16)
            cyanTimer = 3
            check = False
        ...
        if pygame.mouse.get_pos()[0] < 435 and pygame.mouse.get_pos()[1] > 105 and pygame.mouse.get_pos()[1] < 505:
            ...
            PygameCTBF.cursorTextBox((extraInfoDesc[displayExtraInfo]), fontFace3, (0, 0, 0),
        ...
        if (pygame.mouse.get_pos()[0] >= 665 and pygame.mouse.get_pos()[0] <= 790 and pygame.mouse.get_pos()[1] >= 65 and
                pygame.mouse.get_pos()[1] <= 115 and previousClick == False and Click[0]):
    ...
    if bingoCardScreen:
        ...
        if (pygame.mouse.get_pos()[0] > 540 and pygame.mouse.get_pos()[0] < 640 and pygame.mouse.get_pos()[1] > 13 and
                pygame.mouse.get_pos()[1] < 63 and previousClick == False and Click[0]):
            bingoCount = 1
            bingoCardChallengeNamesRandomized = []
            excludedBingoNumbers = []
            xBoxList = []
        ...
        for index, amount in enumerate(bingoCardChallengeNamesRandomized):
            ...
            if xShift > 600:
                xShift = 0
            PygameTWF.renderTextWrap(
                amount, fontFace4, (0, 0, 0), 150, window, 0 + xShift, 65 + yShift, 18)
            xShift = xShift + 150
        yShift = 0
        xShift = 0
        ...
        if (pygame.mouse.get_pos()[0] >= 645 and pygame.mouse.get_pos()[0] <= 745 and pygame.mouse.get_pos()[1] >= 13 and
                pygame.mouse.get_pos()[1] <= 63 and previousClick == False and Click[0]):
            secondScreen = True
            window = pygame.display.set_mode([800, 800])
            xBoxList = []
        ...
        if xWaitTime == 0:
            for index, amount in enumerate(bingoBoxClickX):
                ...
                if (coordinateTuple[0] <= pygame.mouse.get_pos()[0] <= coordinateTuple[1] and coordinateTuple[2] <=
                        pygame.mouse.get_pos()[1] <= coordinateTuple[3] and previousClick == False and Click[0]):
                    xBoxList.append(coordinateTuple)
                for index, amount in enumerate(xBoxList):
                    coordinateTuple2 = amount
                    pygame.draw.line(window, (255, 0, 0), (coordinateTuple2[0], coordinateTuple2[2]), (
                        coordinateTuple2[1], coordinateTuple2[3]), 3)
                    pygame.draw.line(window, (255, 0, 0), (coordinateTuple2[1], coordinateTuple2[2]), (
